...
function mountChatbot() {
  ...
  const speak = (text) => {
    ...
    const selectedVoice = window.speechSynthesis.getVoices().find((voice) => voice.lang.toLowerCase().startsWith(selectedLanguage.slice(0, 2).toLowerCase()));
  ...
  close.addEventListener('click', () => { panel.hidden = true; toggle.setAttribute('aria-expanded', 'false'); input.blur(); });
  language.addEventListener('change', () => { status.textContent = `Language selected: ${language.options[language.selectedIndex].text}`; });
